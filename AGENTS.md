...
3. For any change that creates or modifies SysML v2 textual notation, document
   one SysML validation path: local validation evidence from the Syside Editor or
   `python scripts/validate_sysml.py`, or a request for maintainer-run
   privileged Syside validation after review
...
1. Local validation, if Syside is available, using the Syside Editor or the
   repository wrapper:
...
1. Maintainer-run privileged validation, requested from the pull request after
   initial review, using the `Privileged Syside Validation` workflow.
