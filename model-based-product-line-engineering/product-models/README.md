...
| `aebs_autoware_reference_product.sysml` | hand-authored composite | Composite AEBS reference member: specializes the governed `StandaloneAutowareAEBSReferenceMember` decision and binds its `aebsLogicalSystem`/`aebsSoftware` instances to the canonical `DE4SDV_AEBSLogicalArchitecture::system` and `DE4SDV_AEBSPhysicalSoftwareRealization::physicalSoftware` usages. The binding makes the canonical allocation/evidence trace path explicit; it does not resolve platform-stack variants (see the generated projection) or sensing selections (bill-of-features), and does not make the composite a resolved configured product. |
