...
- ``hasRelevantEvidenceContract`` — Outcome B with the reviewed identity
  basis enforced: only natively verified requirement usages qualify
  (RequirementVerificationMembership anchor, directly or through the
  serialized shadow bridge).

The batch tests:

1.  exactly the four identities comprise c5; the global parity-reviewed set
    is c1(7) + c2(2) + c3(1) + c4(1) + c5(4);
2.  the reviewed rows: dispositions, targets, evidence state, forward-only
    required evidence, populated exact-fit decisions;
...
4.  the hardened traversal laws: Requirement-domain enforcement,
    EvidenceContract identity basis (direct + shadow bridge), candidate-first
    quiet absence, missing-binding fail-closed, MemberProduct exclusion,
    disjointness, no name-based fallback;
...
REPLAY_HEC_POST = 34
REPLAY_HEC_POST_SOURCES = 30
REPLAY_HEC_REJECTED = 106
REPLAY_HEC_QUERY_DOMAIN_VIOLATIONS = 89
REPLAY_HEC_UNVERIFIED_REJECTED = 17
REPLAY_HEC_EVIDENCE_CONTRACT_HOPS = 18
REPLAY_HEC_EVIDENCE_CONTRACT_SOURCES = 16
...
class TestC5ScopeAndCounts:
    ...
    def test_expected_evidence_state_counts(self, inventory):
        """c5 advances evidence maturity only: four rows leave
        repository-evidenced (65 -> 61) and join parity-reviewed (11 -> 15)."""
...
class TestReviewedDecisions:
    ...
    def test_hasrelevantevidencecontract_is_retained_with_identity_basis(
        self, decisions
    ):
        ...
        assert "forward obligation" in text
...
class TestEvidenceContractIdentityBasis:
    """API RequirementUsage does not prove EvidenceContract: only natively
    verified requirement usages qualify (direct anchor or shadow bridge)."""
    ...
    def test_verified_source_direct_anchor_and_shadow_bridge_qualify(self):
        traversal = _traversal(_kernel_bindings())
        elements = _elements()
        hops = traversal.traverse(
            "hasRelevantEvidenceContract", _by_id(elements)["req-1"], elements
        )
        by_object = {hop.api_object["@id"]: hop for hop in hops}
        assert set(by_object) == {"dep-ec-verified", "dep-ec-shadow"}
        assert by_object["dep-ec-verified"].target["@id"] == "ec-1"
        assert by_object["dep-ec-shadow"].target["@id"] == "ec-2"

    def test_unverified_shadow_only_does_not_qualify(self):
        """A ReferenceSubsetting whose declared feature is not anchored by a
        membership proves nothing (the shadow bridge needs the anchor)."""
        traversal = _traversal(_kernel_bindings())
        elements = _elements() + [
    ...
    def test_range_class_gate_keys_on_the_governed_contract(self):
        """The identity-basis enforcement applies to the reviewed
        EvidenceContract range class only; the ontology still declares that
        range for hasRelevantEvidenceContract."""
...
class TestImpactServiceConsumer:
    ...
    def test_impact_edges_reflect_the_narrowed_predicates(self):
        ...
        assert "hasRelevantEvidenceContract" in predicates
        # The unverified requirement-like source never becomes an edge.
        assert all(
            edge["target"] != "req-unverified" for edge in result["edges"]
        )
...
class TestReplayEvidence:
    def test_replay_numbers_are_locked_and_consistent(self):
        ...
        assert REPLAY_HEC_PRE == REPLAY_HEC_POST + REPLAY_HEC_REJECTED
        assert REPLAY_HEC_PRE == 140 and REPLAY_HEC_POST == 34
        assert REPLAY_HEC_QUERY_DOMAIN_VIOLATIONS == 89
        assert REPLAY_HEC_UNVERIFIED_REJECTED == 17
        assert REPLAY_HEC_REJECTED == (
            REPLAY_HEC_QUERY_DOMAIN_VIOLATIONS + REPLAY_HEC_UNVERIFIED_REJECTED
        )
        assert REPLAY_HEC_POST == (
            REPLAY_HEC_EVIDENCE_CONTRACT_HOPS + REPLAY_HEC_ACCEPTANCE_CRITERION_HOPS
        )
        assert REPLAY_HEC_POST_SOURCES == (
            REPLAY_HEC_EVIDENCE_CONTRACT_SOURCES
    ...
    def test_replay_numbers_are_recorded_in_the_decisions_and_review_doc(
        ...
        doc = _review_text()
        for token in ("143", "101", "42", "140", "106", "34", "89"):
        ...
        assert "140" in evidence and "34" in evidence and "89" in evidence
...
class TestDocumentAndBoundaryLaws:
    def test_review_doc_exists_and_names_the_decisions(self):
        ...
        for fragment in (
            ...
            "kernel rule",
            "disjoint",
            "The three modeled evidence-contract links of `reqCommandEmergencyBraking` are preserved",
