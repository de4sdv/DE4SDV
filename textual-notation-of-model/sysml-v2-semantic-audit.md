...
| `textual-notation-of-model/packages/architecture/sdv_platform_stack.sysml` | added in this PR | No SysML v2 model existed for the SDV platform stack architecture or its layer-level variation points. The product-line vision calls for modeling ecosystem diversity (multiple ADAS, middleware, OS, hypervisor, hardware candidates) as configurable architecture variants, and enabling systematic comparison of alternatives. | Adds a cross-cutting architecture package `DE4SDV_SDVPlatformStack` with 5 abstract layer defs (vehicle applications, middleware, OS, hypervisor, hardware), 4 boundary `interface def`s with typed `port def`s for inter-layer contracts, 13 candidate realization `part def`s specializing the layer abstractions, 4 inline `variation part` usages on the stack's layer parts (not orphaned `variation part def`s), 4 `constraint def`s documenting compatibility rules, and `ExampleLinuxSCOREVariant` as a resolved example member product proving the variation mechanism works. No internal architecture of any OSS project is modeled. Constraints are doc-only pending formal constraint expression in a later increment. |
...
- SDV platform stack compatibility constraints are documented as doc comments only. Formal `require constraint` bodies encoding valid/invalid layer combinations depend on SysML v2 constraint expression syntax validation for cross-variant references.
