...
| `textual-notation-of-model/packages/architecture/sdv_platform_stack.sysml` | added in this PR | No SysML v2 model existed for the SDV platform stack architecture or its layer-level variation points. The product-line vision calls for modeling ecosystem diversity (multiple ADAS, middleware, OS, hypervisor, hardware candidates) as configurable architecture variants, and enabling systematic comparison of alternatives. | Adds a cross-cutting architecture package `DE4SDV_SDVPlatformStack` with 5 abstract layer defs (ADAS, middleware, OS, hypervisor, hardware), 4 boundary `interface def`s with typed `port def`s for inter-layer contracts, 13 candidate realization `part def`s specializing the layer abstractions, 4 `variation part def`s enumerating candidates per layer, and 4 `constraint def`s documenting compatibility rules. No internal architecture of any OSS project is modeled. Constraints are doc-only pending formal constraint expression in a later increment. |
