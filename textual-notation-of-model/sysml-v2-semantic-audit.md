...
| `textual-notation-of-model/packages/features/aebs/aebs_needs_requirements.sysml` | open PR #42, repaired in this PR update | Initial PR #42 moved from requirements-as-parts to native requirement notation, but stakeholder needs were still generic parts, V&V/evidence/quality records were modeled as generic parts in the requirements slice, needs were briefly duplicated as a concern usage, satisfaction was asserted against placeholder parts before any functional/conceptual realization existed, and the AEBS common-capability intent was weakened by `ApplicableMemberProduct` subset wording. | Needs are now requirement-like usages only, AEBS is modeled as a common capability required across product-line member products. This increment adds two typed `concern` usages and two concrete `view` usages with targeted `expose`: `aebsStakeholderNeedsView` (stakeholder needs) and `aebsRequirementTraceView` (requirement candidates). V&V/evidence/gap records stay in Markdown/YAML. |
| `textual-notation-of-model/packages/architecture/sdv_platform_stack.sysml` | added in this PR | No SysML v2 model existed for the SDV platform stack architecture or its layer-level variation points. The product-line vision calls for modeling ecosystem diversity (multiple ADAS, middleware, OS, hypervisor, hardware candidates) as configurable architecture variants, and enabling systematic comparison of alternatives. | Adds a cross-cutting architecture package `DE4SDV_SDVPlatformStack` with 6 abstract layer defs (vehicle applications, application-middleware adapter, middleware, OS, hypervisor, hardware), 5 boundary `interface def`s with typed `port def`s for inter-layer contracts, 17 candidate realization `part def`s specializing the layer abstractions, 5 inline `variation part` usages on the stack's layer parts (not orphaned `variation part def`s), 5 `constraint def`s documenting compatibility rules, and `ExampleLinuxSCOREVariant` as a resolved example member product proving the variation mechanism works. No internal architecture of any OSS project is modeled. Constraints are doc-only pending formal constraint expression in a later increment.
...
- **satisfaction assertions** as deferred until a later functional/conceptual realization model contains concrete satisfying features.
