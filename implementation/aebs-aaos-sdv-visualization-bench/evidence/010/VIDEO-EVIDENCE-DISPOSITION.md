# INC-AEBS-010 external video disposition index

No video bytes are retained in `evidence/010/`. Their authoritative checksum,
byte count, availability, former path, and disposition are maintained in
[`external-media.yaml`](external-media.yaml). Statuses follow the evidence vocabulary of
...
| Artifact | Former repository path | Disposition | Role/availability |
|---|---|---|---|
| Final publication cut | `forward-ui/final-hmi-v21-corrected/final-cut-v2.mp4` | `publication_cut` — 26.1 s: 3 s editing title card, 20 s untouched real-time take (0–20 s of the raw below), 3 s editing end card | Maintainer archive; not public evidence while no controlled external location is recorded |
| Continuous raw runtime recording | `forward-ui/final-hmi-v21-corrected/raw-continuous.mp4` | `observed_bounded` — 70.15 s, 1080×600, untrimmed single take of the corrected final UI generation | Maintainer archive; correlated machine-readable records remain in Git |
...
| Correlated logs + disposition | `forward-ui/final-hmi-v21-corrected/` (`segment.yaml`, `revision-checksums.md`, `app-ingress-frame.log`, `observer.log`, `observer-raw.json`, capture timestamps, guest logcat) | `observed_bounded` with `scenario_safety_outcome: deferred_not_proven` | Empty bridge/bench journal files were removed because zero bytes carry no observation. |
| Editing card assets | `forward-ui/final-hmi-v21-corrected/card-title-v2.mp4`, `card-end-v2.mp4` | `publication_cut` component — 3 s editing frames only, not app output | Maintainer archive; never evidence of runtime behavior |
...
decision pair. Their identities are retained in the external-media manifest for the correction
trail only. They must not be published, embedded in docs or posts, or cited as
current HMI evidence.

| Artifact | Former repository path | HMI generation | Obsolete/misleading semantics | Disposition |
|---|---|---|---|---|
...
- Obsolete videos were removed from the current tree with checksums and reasons
  retained in `external-media.yaml`. Historical Git objects and the verified
  pre-change mirror preserve the correction trail.
