...
# Usage: run_degraded_input_profile.sh PROFILE
#   PROFILE — stale_input | missing_input | malformed_input | inconsistent_input | unavailable_input
set -euo pipefail

BENCH="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PROFILE="${1:-}"
case "$PROFILE" in
  stale_input|missing_input|malformed_input|inconsistent_input|unavailable_input) ;;
  *) printf 'Usage: %s PROFILE (stale_input|missing_input|malformed_input|inconsistent_input|unavailable_input)\n' "$0" >&2; exit 2 ;;
esac
EVIDENCE="$BENCH/evidence/009f/$PROFILE"
RUNS="$EVIDENCE/runs"
...
if [ -L "$EVIDENCE" ] || [ ! -d "$EVIDENCE" ]; then
  printf 'Unsafe degraded-input evidence directory: %s\n' "$EVIDENCE" >&2
  exit 1
fi
...
if root.is_symlink() or runs.is_symlink() or not runs.is_dir():
 raise SystemExit("unsafe degraded-input evidence/run directory")
...
DE4SDV_009F_PROFILE="$PROFILE" DE4SDV_009F_RUN_DIR="$STAGE" "$BENCH/scripts/launch.sh"
...
SCENARIO_CONFIG_PATH="/de4sdv/implementation/aebs-autoware-nominal-vehicle-target-bench/workspace/install/de4sdv_aebs_009b_bench/share/de4sdv_aebs_009b_bench/config/scenario-009f-degraded-input-matrix.yaml"
set +e
...
' "$FAILURE" "$observer_exit" "evidence/009f/'"$PROFILE"'/runs/$RUN_ID/observer-raw.json"
...
bench=pathlib.Path(sys.argv[1]); profile=sys.argv[2]
config_path="config/scenario-009f-degraded-input-matrix.yaml"
value=_live_provenance_fields(bench); value.update(captured_utc=datetime.now(timezone.utc).isoformat().replace("+00:00","Z"),command_exit_code=int(sys.argv[3]),degraded_input_config_sha256=sha256_file(bench/config_path))
...
' "$BENCH" "$PROFILE" "$observer_exit" "$PROVENANCE"
...
bench=pathlib.Path(sys.argv[1]); run_id=sys.argv[3]; profile=sys.argv[4]; records={}
for name,leaf in (("observer_raw","observer-raw.json"),("observer_log","observer.log"),("launch_log","launch.log"),("run_metadata","run-metadata.json"),("map_runtime","map-runtime.json")):
 relative=f"evidence/009f/{profile}/runs/{run_id}/{leaf}"; path=bench/relative
...
python3 "$BENCH/scripts/degraded_input_evidence.py" \
...
python3 "$BENCH/scripts/validate_degraded_input_evidence.py" --candidate --bench-root "$BENCH" --profile "$PROFILE" "$STAGED"
PYTHONPATH="$BENCH/scripts${PYTHONPATH:+:$PYTHONPATH}" python3 -c '
import pathlib,sys
...
printf 'Retained replay-validated 009F %s evidence: %s\n' "$PROFILE" "$CANONICAL"
