...
def _verify_map_runtime(
    ...
    try:
        ...
        raise ValidationError(f"cannot read extracted-map identities from runtime lock: {error}") from error
...
def validate_evidence(
    ...
    *,
    ...
    config_path = Path(scenario_config) if scenario_config else root / "config" / "scenario-009b-moving-vehicle-target.yaml"
