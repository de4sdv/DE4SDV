...
# Usage: run_crossing_target_scenario.sh INCREMENT SCENARIO
#   INCREMENT — INC-AEBS-009G or INC-AEBS-009H
#   SCENARIO  — pedestrian-crossing or bicycle-crossing
set -euo pipefail
...
SCENARIO="${2:-}"
case "$INCREMENT" in
  INC-AEBS-009G|INC-AEBS-009H) ;;
  *) printf 'Usage: %s INCREMENT SCENARIO (INC-AEBS-009G|INC-AEBS-009H pedestrian-crossing|bicycle-crossing)\n' "$0" >&2; exit 2 ;;
esac
case "$SCENARIO" in
  pedestrian-crossing|bicycle-crossing) ;;
  *) printf 'Usage: %s INCREMENT SCENARIO (INC-AEBS-009G|INC-AEBS-009H pedestrian-crossing|bicycle-crossing)\n' "$0" >&2; exit 2 ;;
esac
# Derive the evidence subdirectory from the increment.
EVIDENCE_SUBDIR="$(printf '%s' "$INCREMENT" | sed 's/^INC-AEBS-//' | tr '[:upper:]' '[:lower:]')"
EVIDENCE="$BENCH/evidence/$EVIDENCE_SUBDIR"
RUNS="$EVIDENCE/runs"
...
if [ -L "$EVIDENCE" ] || [ ! -d "$EVIDENCE" ]; then
  printf 'Unsafe crossing-target evidence directory: %s\n' "$EVIDENCE" >&2
  exit 1
fi
...
if root.is_symlink() or runs.is_symlink() or not runs.is_dir():
 raise SystemExit("unsafe crossing-target evidence/run directory")
...
DE4SDV_009G_SCENARIO="$SCENARIO" DE4SDV_009G_RUN_DIR="$STAGE" "$BENCH/scripts/launch.sh"
...
SCENARIO_CONFIG_PATH="/de4sdv/implementation/aebs-autoware-nominal-vehicle-target-bench/workspace/install/de4sdv_aebs_009b_bench/share/de4sdv_aebs_009b_bench/config/scenario-009${SCENARIO#pedestrian-crossing}.yaml"
# Map scenario name to config file: pedestrian-crossing -> 009g, bicycle-crossing -> 009h
case "$SCENARIO" in
  pedestrian-crossing) SCENARIO_CONFIG_PATH="${SCENARIO_CONFIG_PATH%009crossing.yaml}009g-pedestrian-crossing.yaml" ;;
  bicycle-crossing)    SCENARIO_CONFIG_PATH="${SCENARIO_CONFIG_PATH%009crossing.yaml}009h-bicycle-crossing.yaml" ;;
esac
set +e
docker exec --user "$(id -u):$(id -g)" --env HOME=/home/aw "$CONTAINER" bash -lc '
  ...
  exec timeout --signal=TERM "$1" ros2 run de4sdv_aebs_009b_bench scenario_observer \
    ...
    -p "raw_output:=$3" \
    -p "timeout_s:=$4" \
    -p "crossing_target_scenario:=$5"
' observer "$SUPERVISOR_TIMEOUT" \
  "$SCENARIO_CONFIG_PATH" \
  "$CONTAINER_RAW" "$SCENARIO_TIMEOUT" "$SCENARIO" >&"$observer_fd" 2>&1
observer_exit=$?
set -e
exec {observer_fd}>&-
...
' "$FAILURE" "$observer_exit" "evidence/$EVIDENCE_SUBDIR/runs/$RUN_ID/observer-raw.json"
...
bench=pathlib.Path(sys.argv[1]); increment=sys.argv[3]; config_map={"INC-AEBS-009G":"config/scenario-009g-pedestrian-crossing.yaml","INC-AEBS-009H":"config/scenario-009h-bicycle-crossing.yaml"}
value=_live_provenance_fields(bench); value.update(captured_utc=datetime.now(timezone.utc).isoformat().replace("+00:00","Z"),command_exit_code=int(sys.argv[2]),crossing_config_sha256=sha256_file(bench/config_map[increment]))
fd,name=tempfile.mkstemp(prefix=".provenance.",dir=pathlib.Path(sys.argv[4]).parent)
...
os.replace(name,sys.argv[4])
' "$BENCH" "$observer_exit" "$INCREMENT" "$PROVENANCE"
...
bench=pathlib.Path(sys.argv[1]); run_id=sys.argv[3]; subdir=sys.argv[4]; records={}
for name,leaf in (("observer_raw","observer-raw.json"),("observer_log","observer.log"),("launch_log","launch.log"),("run_metadata","run-metadata.json"),("map_runtime","map-runtime.json")):
 relative=f"evidence/{subdir}/runs/{run_id}/{leaf}"; path=bench/relative
...
' "$BENCH" "$FINAL/artifacts.json" "$RUN_ID" "$EVIDENCE_SUBDIR"

python3 "$BENCH/scripts/crossing_target_evidence.py" \
  --bench-root "$BENCH" --raw "$FINAL/observer-raw.json" \
  --provenance "$FINAL/provenance.json" --artifacts "$FINAL/artifacts.json" \
  --increment "$INCREMENT" --output "$STAGED"
python3 "$BENCH/scripts/validate_crossing_target_evidence.py" --candidate --bench-root "$BENCH" --increment "$INCREMENT" "$STAGED"
PYTHONPATH="$BENCH/scripts${PYTHONPATH:+:$PYTHONPATH}" python3 -c '
import pathlib,sys
