...
PROFILE="${1:-}"
case "$PROFILE" in
  clear_path|adjacent_object|non_closing_target|below_trigger) ;;
  *) printf 'Usage: %s PROFILE (one closed 009E profile)\n' "$0" >&2; exit 2 ;;
esac
EVIDENCE="$BENCH/evidence/009e/profiles/$PROFILE"
RUNS="$EVIDENCE/runs"
...
if [ -L "$EVIDENCE" ] || [ ! -d "$EVIDENCE" ]; then
  printf 'Unsafe 009E evidence directory: %s\n' "$EVIDENCE" >&2
  exit 1
fi
...
if root.is_symlink() or runs.is_symlink() or not runs.is_dir():
 raise SystemExit("unsafe 009E evidence/run directory")
if not runs.resolve(strict=True).is_relative_to(root.resolve(strict=True)):
 raise SystemExit("009E run directory escapes evidence root")' "$EVIDENCE" "$RUNS"
...
DE4SDV_009E_PROFILE="$PROFILE" DE4SDV_009E_RUN_DIR="$STAGE" "$BENCH/scripts/launch.sh"
...
SCENARIO_CONFIG_PATH="/de4sdv/implementation/aebs-autoware-nominal-vehicle-target-bench/workspace/install/de4sdv_aebs_009b_bench/share/de4sdv_aebs_009b_bench/config/scenario-009e-${PROFILE//_/-}.yaml"
set +e
...
write(pathlib.Path(sys.argv[1]),{"observer_exit_code":int(sys.argv[2]),"raw_output":sys.argv[3],"non_activation_profile":sys.argv[4]})
' "$FAILURE" "$observer_exit" "evidence/009e/profiles/$PROFILE/runs/$RUN_ID/observer-raw.json" "$PROFILE"
...
value=_live_provenance_fields(bench); value.update(captured_utc=datetime.now(timezone.utc).isoformat().replace("+00:00","Z"),command_exit_code=int(sys.argv[3]),non_activation_profile=profile,non_activation_execution_manifest_sha256=non_activation_execution_manifest_sha256(bench,profile),non_activation_matrix_sha256=sha256_file(bench/"config/scenario-009e-non-activation-matrix.yaml"))
...
for name,leaf in (("observer_raw","observer-raw.json"),("observer_log","observer.log"),("launch_log","launch.log"),("run_metadata","run-metadata.json"),("map_runtime","map-runtime.json")):
 relative=f"evidence/009e/profiles/{sys.argv[4]}/runs/{run_id}/{leaf}"; path=bench/relative
...
python3 "$BENCH/scripts/non_activation_evidence.py" \
...
python3 "$BENCH/scripts/validate_non_activation_evidence.py" --candidate --bench-root "$BENCH" "$STAGED"
PYTHONPATH="$BENCH/scripts${PYTHONPATH:+:$PYTHONPATH}" python3 -c '
import pathlib,sys
...
printf 'Retained replay-validated 009E profile %s evidence: %s\n' "$PROFILE" "$CANONICAL"
