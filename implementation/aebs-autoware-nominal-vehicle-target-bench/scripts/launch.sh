...
if [ -L "$run_dir" ] || [ ! -d "$run_dir" ]; then
  printf 'Unsafe or missing 009B run directory: %s\n' "$run_dir" >&2
  exit 1
fi
python3 -c 'import pathlib,sys
bench=pathlib.Path(sys.argv[1]).resolve(strict=True)
run=pathlib.Path(sys.argv[2])
if run.is_symlink() or not run.is_dir() or not run.resolve(strict=True).is_relative_to(bench/"evidence"/"009b"):
 raise SystemExit("unsafe 009B run directory")' "$BENCH" "$run_dir"
...
docker compose -f "$BENCH/compose.yaml" run --rm --name "$runtime_name" bench bash -lc '
  ...
  set -u
  exec ros2 launch de4sdv_aebs_009b_bench aebs_009b_bench.launch.py \
...
printf '009B process started as PID %s; this is not readiness, scenario, safety, or compliance evidence.\n' "$pid"
