"""ROS adapter for the manually activated, moving 009B target fixture.
...
class ScenarioFixture(Node):
    ...
    def __init__(self) -> None:
        ...
        self.control_pub = self.create_publisher(
            Control, "/de4sdv/aebs_009b/nominal_control_cmd", 1
        )
        self.override_pub = self.create_publisher(
            BoolStamped, "/de4sdv/aebs_009b/driver_override", 1
        )
        self.availability_pub = self.create_publisher(OperationModeAvailability, "/system/operation_mode/availability", 1)
    ...
    def _on_odometry(self, message: Odometry) -> None:
        ...
        try:
            ...
            self._ego_speed_mps = float(message.twist.twist.linear.x)
    ...
    def _on_activate(self, _request: Trigger.Request, response: Trigger.Response):
        try:
            anchored = self._state.activate(time.monotonic())
        ...
        response.message = "moving same-lane map target activated"
        return response

    def _publish_cloud(self) -> None:
        try:
            now_s = time.monotonic()
            points = self._state.target_points(now_s)
        ...
        moving_pose = self._state.target_pose_map(now_s)
        if moving_pose is not None:
            target = PoseStamped()
            target.header.stamp = cloud.header.stamp
            target.header.frame_id = "map"
            _set_planar_pose(target.pose, moving_pose)
            self.target_pose_pub.publish(target)
    ...
    def _publish_nominal_inputs(self) -> None:
        ...
        control.longitudinal.acceleration = nominal_acceleration_for_speed(
            config, self._ego_speed_mps
        )
