"""ROS-independent state machine for the one-authority 009B moving-target fixture."""
...
def nominal_acceleration_for_speed(config: ScenarioConfig, current_speed_mps: float) -> float:
    """Use configured launch acceleration only below the nominal speed setpoint."""

    current = _require_float32("current_speed_mps", current_speed_mps)
    return config.nominal_command_acceleration_mps2 if current < config.nominal_command_speed_mps else 0.0
...
class ScenarioFixtureState:
    ...
    def __init__(self, config: ScenarioConfig) -> None:
        ...
        self._activation_monotonic_s: float | None = None
    ...
    def activate(self, monotonic_s: float = 0.0) -> Pose2D:
        ...
        self._activation_monotonic_s = float(monotonic_s)
        return self._anchored_target_pose_map

    def target_pose_map(self, monotonic_s: float = 0.0) -> Pose2D | None:
        if self._anchored_target_pose_map is None:
            return None
        elapsed = max(0.0, float(monotonic_s) - float(self._activation_monotonic_s))
        travel = self._config.target_speed_mps * elapsed
        anchor = self._anchored_target_pose_map
        return Pose2D(
            anchor.x + travel * math.cos(anchor.yaw_rad),
            anchor.y + travel * math.sin(anchor.yaw_rad),
            anchor.yaw_rad,
        )

    def target_points(self, monotonic_s: float = 0.0) -> tuple[Point3D, ...]:
        target_pose_map = self.target_pose_map(monotonic_s)
        if target_pose_map is None:
            return ()
        if self._ego_pose_map is None:
            raise RuntimeError("activated target requires current ego pose")
        target_base_link = map_pose_to_base_link(target_pose_map, self._ego_pose_map)
