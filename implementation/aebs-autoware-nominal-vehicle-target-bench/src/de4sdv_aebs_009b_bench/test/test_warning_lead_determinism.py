...
def test_warning_evaluable_before_latch_transition_when_geometry_present():
    ...
    warning = warning_on_intervention_diagnostic(
        False, latch.state, rss_distance_m=23.45, point_distance_m=23.80, warning_margin_m=7.0
    )
...
def test_no_warning_fabricated_without_geometry_inputs():
    ...
    latch = InterventionLatch(stop_speed_mps=0.1, stop_hold_s=0.5, odometry_max_age_s=0.2)
    warning = warning_on_intervention_diagnostic(
        False, latch.state, rss_distance_m=None, point_distance_m=None, warning_margin_m=7.0
    )
...
def test_warning_condition_absent_at_latch_stays_absent():
    ...
    warning = warning_on_intervention_diagnostic(
        False, latch.state, rss_distance_m=10.0, point_distance_m=63.74, warning_margin_m=7.0
    )
    assert warning is False
    latch.observe_diagnostic(True, True)
    assert latch.state == "braking_latched"
...
def test_static_velocity_model_param_is_pinned():
    """The fixture pins use_object_velocity_calculation: false so the native RSS
    depends only on ego kinematics (deterministic warning-lead schedule)."""
    text = (BENCH_ROOT / "config" / "aebs-009b.param.yaml").read_text()
    match = next(
        line for line in text.splitlines()
        if line.strip().startswith("use_object_velocity_calculation:")
    )
    assert match.strip() == "use_object_velocity_calculation: false"
