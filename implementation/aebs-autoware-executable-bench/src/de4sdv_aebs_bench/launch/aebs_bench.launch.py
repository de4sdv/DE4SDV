"""INC-AEBS-009A headless Autoware AEBS integration-bench composition.

This launch assembles the pinned map, simulator, AEB, diagnostic, MRM, and
vehicle-gate packages. It proves launch/readiness only; scenario stimulus and
braking assertions remain INC-AEBS-009B/009C.
"""
...
def generate_launch_description():
    package_share = get_package_share_directory("de4sdv_aebs_bench")
    ...
    operator_share = get_package_share_directory("autoware_mrm_emergency_stop_operator")
    ...
    map_loader = include(
        ...
        "launch/map.launch.xml",
        {
            ...
            "pointcloud_map_loader_param_path": (
                f"{package_share}/config/pointcloud_map_loader.param.yaml"
            ),
            "lanelet2_map_loader_param_path": (
                f"{package_share}/config/lanelet2_map_loader.param.yaml"
            ),
            "map_tf_generator_param_path": (
                f"{package_share}/config/map_tf_generator.param.yaml"
            ),
            "map_projection_loader_param_path": (
                f"{package_share}/config/map_projection_loader.param.yaml"
            ),
        },
    )
    ...
    simulator = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            # Required because ParameterFile substitution evaluates an unused
            # $(var vehicle_model_pkg) token inside a pinned YAML comment.
            "vehicle_model_pkg": vehicle_info_share,
        }.items(),
    )

    diagnostic_graph = include(
        "autoware_diagnostic_graph_aggregator",
        "launch/aggregator.launch.xml",
        {"graph_file": f"{package_share}/config/diagnostic-graph.yaml"},
    )
    mrm_handler = include("autoware_mrm_handler", "launch/mrm_handler.launch.xml")
    # Direct construction prevents its generic `config_file` launch argument
    # colliding with the MRM handler include's argument of the same name.
    emergency_stop = Node(
        ...
        output="screen",
        parameters=[f"{operator_share}/config/mrm_emergency_stop_operator.param.yaml"],
        remappings=[
    ...
    # The standalone upstream gate wrapper omits operation-mode, kinematics,
    # acceleration, and operation-mode-output remaps. Instantiate the selected
    # legacy gate directly using the pinned full control-launch contract.
    vehicle_gate = Node(
    ...
    # Direct instantiation avoids the pinned upstream wrapper's undeclared
    # odometry remap; the AEB node has no such declared input.
    aeb = Node(
        ...
        output="screen",
        parameters=[
            f"{package_share}/config/aebs.param.yaml",
            f"{vehicle_info_share}/config/vehicle_info.param.yaml",
        ],
        remappings=[
    ...
    fixture = Node(
        ...
        executable="nominal_fixture",
        name="nominal_fixture",
        output="screen",
    )
