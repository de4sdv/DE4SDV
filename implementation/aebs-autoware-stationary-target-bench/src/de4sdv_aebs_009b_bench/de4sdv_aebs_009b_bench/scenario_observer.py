...
from tier4_system_msgs.msg import MrmBehaviorStatus, OperationModeAvailability
from tier4_vehicle_msgs.msg import VehicleEmergencyStamped

from .scenario_contract import BASELINE_REQUIRED_INPUTS, load_scenario_config
...
_TOPIC_KIND = {
    ...
    "/system/fail_safe/mrm_state": ObservationKind.MRM_STATE,
    "/system/mrm/emergency_stop/status": ObservationKind.EMERGENCY_OPERATOR_STATUS,
...
class ScenarioObserver(Node):
    ...
    def __init__(self) -> None:
        ...
        self.create_subscription(MrmState, "/system/fail_safe/mrm_state", self._safe(self._mrm, "/system/fail_safe/mrm_state"), 10)
        self.create_subscription(MrmBehaviorStatus, "/system/mrm/emergency_stop/status", self._safe(self._operator, "/system/mrm/emergency_stop/status"), 10)
    ...
    def _mrm(self, message: MrmState, at: float) -> None:
        state = closed_constant(MrmState, int(message.state), ("NORMAL", "MRM_OPERATING", "MRM_SUCCEEDED", "MRM_FAILED"))
        behavior = closed_constant(MrmState, int(message.behavior), ("NONE", "EMERGENCY_STOP"))
        self.core.add(Observation(ObservationKind.MRM_STATE, {"state": state, "behavior": behavior}, at))

    def _operator(self, message: MrmBehaviorStatus, at: float) -> None:
        state = closed_constant(
            MrmBehaviorStatus,
            int(message.state),
            ("NOT_AVAILABLE", "AVAILABLE", "OPERATING"),
        )
        self.core.add(Observation(ObservationKind.EMERGENCY_OPERATOR_STATUS, {"state": state}, at))
    ...
    def _odometry(self, message: Odometry, at: float) -> bool:
        item = self.core.make_odometry(float(message.twist.twist.linear.x), at, _stamp(message))
        if item is None:
            return False
        self.core.add(item)
        return True
    ...
    def _target(self, message: PoseStamped, at: float) -> None:
        ...
        position, q = message.pose.position, message.pose.orientation
        ...
        if not math.isclose(q.x, 0.0, abs_tol=1e-6) or not math.isclose(q.y, 0.0, abs_tol=1e-6):
            raise ValueError("target quaternion must be planar")
        ...
        self.core.add(Observation(ObservationKind.TARGET_PUBLICATION, {
            "identity": "target-1", "frame": "map", "x": values[0], "y": values[1], "yaw_rad": yaw,
        }, at, source_stamp=_stamp(message)))
