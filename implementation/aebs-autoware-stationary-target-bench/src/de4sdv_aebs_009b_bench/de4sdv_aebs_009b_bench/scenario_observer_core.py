...
class ObserverCore:
    ...
    def make_odometry(self, speed: float, at: float, source_stamp: str | None) -> Observation | None:
        ...
        return Observation(ObservationKind.ODOMETRY, {
            "speed_mps": float(speed), "acceleration_mps2": acceleration[0],
    ...
    def poll_terminal(self, now_s: float) -> str | None:
        ...
        if result.outcome is Outcome.FAIL_SCENARIO:
            ...
            missing = (
                result.reasons[0].startswith("Required event ")
