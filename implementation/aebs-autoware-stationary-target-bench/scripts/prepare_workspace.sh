...
mkdir -p "$BENCH/workspace/src" "$BENCH/evidence/009c"
docker compose -f "$BENCH/compose.yaml" run --rm bench bash -lc '
  ...
  ln -sfn /de4sdv/implementation/aebs-autoware-stationary-target-bench/src/de4sdv_aebs_009c_bench src/de4sdv_aebs_009c_bench
  test "$(find src -mindepth 1 -maxdepth 1 -printf "." | wc -c)" -eq 1
  test -L src/de4sdv_aebs_009c_bench
'
printf '009C overlay workspace prepared; no upstream source was imported.\n'
