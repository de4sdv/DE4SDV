...
| `ExtendedRequirement` | `DE4SDV_SYSMODAdapter::SYSMODRequirementBase` is the package seam. `DE4SDV_MethodContext::RequirementCandidate` specializes it; candidate lifecycle semantics remain local. | Needs are Operational inputs; design-input requirements are handled through Conceptual viewpoints. | Migrated: `RequirementCandidate` inherits obligation, stability, and motivation metadata. |
| `SystemUseCase` | `DE4SDV_SYSMODAdapter::SYSMODSystemUseCaseBase` is the package seam. AEBS `MitigateVehicleTargetForwardCollisionRisk` and middleware `IntegrateADASWithVehiclePlatform` specialize it. | Operational scenarios motivate system use cases; use-case artifacts participate in Conceptual views. | Migrated: both use cases populate motivation, trigger, and result semantics. |
